...
## Свидания за токены (крипто-эскроу)

Смарт-контракт-эскроу на локальном Hardhat: пользователь предлагает свидание за токены
`DATE`, при согласии средства замораживаются, при подтверждении 80% уходят партнёру и
20% — сервису; при отмене после согласия удерживается 20% штрафа. Кошельки кастодиальные
(демо), логика — на контракте `DateEscrow`. Подробности: [docs/04-CRYPTO-DATES.md](docs/04-CRYPTO-DATES.md).

Запуск цепочки (в отдельных терминалах):

```bash
pnpm chain:node      # локальный узел Hardhat на :8545
pnpm chain:deploy    # деплой DateToken+DateEscrow → packages/contracts/deployments/localhost.json
pnpm chain:test      # unit-тесты контрактов (18 шт.)
# затем обычный запуск api/user-web/admin-web — API прочитает deployments на старте
```

Поток: Discovery → «Предложить свидание за токены» → у второго во вкладке «Свидания»
кнопки «Согласиться/Отклонить» → инициатор «Свидание состоялось» (перевод 80/20) или
«Отменить». Сервисный кошелёк и комиссии — в admin-web → «Кошелёк сервиса».
