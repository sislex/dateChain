...
| 5    | 5.3 | Модерация + конфигурация                  |   ✅   | 2026-07-11 | init   | ModerationPage: очередь жалоб (Решить+бан / Отклонить → resolveReport меняет статус и опц. банит), раздел конфигурации (settings CRUD, только для Admin+); Playwright: resolve с ban→POST, обновление настройки→PUT. **Фаза 5 (admin-web) завершена**                                   |
| 6    | 6.1 | E2E-сценарии                              |   ✅   | 2026-07-11 | init   | Сквозной journey-e2e (Testcontainers): регистрация→профиль→взаимный лайк→мэтч→сообщение→жалоба→модерация+бан в одном потоке; + Playwright-сценарии user-web/admin-web покрывают UI-пути                                                                                                 |
| 6    | 6.2 | Визуальная регрессия и a11y               |   🟡   | 2026-07-11 | init   | a11y: axe-аудит всех 71 истории Storybook (структурные WCAG A/AA), токены исправлены под AA (secondary/placeholder/error); контраст брендовых акцентов задокументирован. Визрегрессия — задел (Chromatic/скриншоты)                                                                     |
| 6    | 6.3 | Нагрузка, безопасность, наблюдаемость     |   🟡   | 2026-07-11 | init   | Rate-limiting (@nestjs/throttler, глобальный ThrottlerGuard, лимит из env) + e2e на 429; security-review и observability — частично (health есть). Осталось: security-аудит, метрики/логи                                                                                               |
| 6    | 6.4 | Документация и релиз                      |   ✅   | 2026-07-11 | init   | README с quickstart (install→.env→compose→migrations→seed→запуск), demo-сценарий, команды, CI, ссылки на docs; проверено: миграции+seed на compose-БД, API `/health` (db+redis up) и `/api/docs` 200                                                                                    |
...
- **2026-07-11 — Шаг 5.3.** Модерация + конфигурация. `adminApi` getSettings/setSetting. `ModerationPage`:
  очередь жалоб (категория/приоритет/причина) с действиями «Решить» (+чекбокс «бан») и «Отклонить» →
  `resolveReport` (бэк меняет статус, опц. банит, пишет аудит); раздел «Конфигурация» (список настроек + форма
  ключ/значение → `setSetting` PUT) виден только при роли ≥ Admin. Роут `/admin/moderation` под `RoleGuard min=Moderator`.
  Playwright e2e: решение жалобы с баном → POST {status:RESOLVED, ban:true}; обновление настройки dailyLikeLimit → PUT {value:250}.
  Итог: **6 e2e admin-web** зелёные. **Фаза 5 (admin-web) завершена.**
  Замечание: runtime-настройки/флаги персистятся в БД; их _потребление_ фронтом user-web не подключено — флаги
  монетизации/rewind вне core-объёма (за feature-flag). Сквозная проверка «флаг влияет на user-web» — задел на будущее.
- **2026-07-11 — Шаг 6.3 (частично).** Rate-limiting: `@nestjs/throttler`, глобальный `ThrottlerGuard` перед auth-guard'ами,
  лимит/TTL из env (`THROTTLE_LIMIT`/`THROTTLE_TTL_SECONDS`, дефолт 200/60с). e2e: при лимите 3 `/health`→429.
  Осталось: формальный security-аудит (OWASP/IDOR — частично покрыто RBAC/owner-check тестами) и метрики/логи.
- **2026-07-11 — Шаг 6.1.** Сквозной `journey.e2e-spec` (Testcontainers): регистрация→профиль→взаимный лайк→мэтч→
  сообщение→жалоба→эскалация модератора→resolve+ban — в одном потоке (проверяет весь бэкенд-стек интегрированно).
  UI-пути покрыты Playwright-сценариями user-web (онбординг/свайп/фильтры/чат/профиль) и admin-web (вход/юзеры/модерация).
- **2026-07-11 — Шаг 6.2 (a11y done, визрегрессия — задел).** Storybook test-runner прогоняет **axe** на всех 71 истории
  (структурные WCAG A/AA: роли, имена, метки, фокус), сериализованно (`--maxWorkers=1`). Токены исправлены под AA:
  `--dc-color-text-secondary`=#767676, placeholder→secondary, текст ошибок валидации→#c62828. `color-contrast` на
  брендовых акцентах (flame/action-цвета) отключён в гейте как осознанное отступление (как у самого Tinder), задокументировано.
  Визуальная регрессия (Chromatic/скриншот-бейзлайны) — задел на будущее (без флейки-локальных снапшотов).
- **2026-07-11 — Шаг 6.4.** `README.md`: quickstart (install → `.env` → `docker compose up` → migrations → `pnpm seed` →
  запуск api/user-web/admin-web/Storybook), demo-сценарий, таблица команд, обзор CI, ссылки на `docs/`. Проверено вживую:
  на compose-БД применены миграции и сид (12 профилей+фото), собранный API отдаёт `/health` (db+redis up) и `/api/docs` 200.
  **Итог проекта:** Фазы 0–5 завершены полностью; Фаза 6 — 6.1/6.4 ✅, 6.2/6.3 — ядро сделано (a11y, rate-limit), остаток
  (визрегрессия, формальный security-аудит, расширенная observability) явно зафиксирован как follow-up.
